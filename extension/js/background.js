// Egg TV Extension Background Script
...
chrome.runtime.onInstalled.addListener(async (details) => {
    if (details.reason === 'install') {
        ...
        chrome.notifications.create({
            ...
            title: 'Egg TV Toolbar Installed',
...
chrome.action.onClicked.addListener(async (tab) => {
    try {
        ...
        console.error('Egg TV: Failed to toggle toolbar:', error);
    }
});
...
async function updateBadge() {
    try {
        ...
        console.error('Egg TV: Failed to update badge:', error);
    }
}
...
chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs.length > 0) {
        ...
        console.log('Egg TV: Initialized with active tab', currentTabId);
    }
});
...
chrome.tabs.onCreated.addListener(async (tab) => {
    ...
    if (currentTabId && currentTabId !== tab.id) {
        try {
            ...
            console.log('Egg TV: Closed previous tab', currentTabId, 'for new tab', tab.id);
        } catch (error) {
            // Tab might already be closed, ignore error
            console.log('Egg TV: Previous tab already closed');
        }
    }
    // Set the new tab as the current tab
    currentTabId = tab.id;
    console.log('Egg TV: Now tracking tab', tab.id);
});
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (changeInfo.url && tabId === currentTabId) {
        console.log('Egg TV: Tab navigated to', changeInfo.url);
    }
});
...
chrome.tabs.onRemoved.addListener((tabId) => {
    if (tabId === currentTabId) {
        currentTabId = null;
        console.log('Egg TV: Stopped tracking removed tab', tabId);
    }
});
